...
# CHALLENGE
level = 0
target_pos = [(0.36, -0.54), (0.36, -0.30),  (0.36, 0.42)]
level_pos = [(-0.12, 0.06), (-0.12, 0.42), (-0.12, 0.18)]
rotations = [[0,1,0,-math.pi/2], [0,1,0,0], [0,1,0,-math.pi/2]]
    ...
    def update(self) -> None:
        ...
        if self.robot_obj.in_simulation:
            ...
            if self._game_state == GameState.MATCH_RUNNING:
                ...
                global target_pos
                global rotations
                # Load new World if Robot finished
                #take robot position x and z
                current_pos = self.robot_obj.position[0::2]
                if get_dist(current_pos, target_pos[level]) < 0.02 and self._game_state == GameState.MATCH_RUNNING:
                    # Set position and rotation of robot
                    self.robot_obj.position = [level_pos[level][0],
                                            -0.03,
                                            level_pos[level][1]]
                    self.robot_obj.rotation = rotations[level]

                    # Reset physics
                    self.robot_obj.reset_physics()

                    level = (level + 1) % len(level_pos)
