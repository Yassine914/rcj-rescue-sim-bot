...
def detect(img):
    ...
    # FIXME:
    if sign_type == 'N':
        sign_type = detect_letters_old(img , camera_left)
...
def detect_shape1(sign_colored, camera) -> str:
    ...
    red_mask = cv.inRange(hsv, (0, 120, 100), (10, 255, 255)) | cv.inRange(hsv, (160, 120, 100), (180, 255, 255))
    yellow_mask = cv.inRange(hsv, (20, 120, 100), (30, 255, 255))
    white_mask = cv.inRange(gray, 200, 255)
    black_mask = cv.inRange(gray, 0, 50)
    ...
    for cnt in contours:
        if cv.contourArea(cnt) < 0.05 * roi_area:
            continue
...
def detect_letters_old(sign, camera) -> str:
    # if detect_shape1(sign, camera) == 'N' or detect_shape1(sign, camera) == 'R':
        # return 'N'
    img = cv.cvtColor(sign, cv.COLOR_BGR2GRAY)
    _, thresh = cv.threshold(img, 80, 255, cv.THRESH_BINARY_INV)
    ...
    if num_contours_first == 3 and num_contours_middle == 3 and num_contours_third == 3:
        return 'U'
    elif num_contours_first == 1 and num_contours_third == 1:
        return 'H'
    elif num_contours_first == 3 and num_contours_third == 1:
        return 'H'
        ...
        return 'S'
    elif num_contours_first == 4 and num_contours_third == 2:
        return 'S'
    elif num_contours_first == 2 and num_contours_third == 2:
        return 'S'
    elif num_contours_first == 2 and num_contours_third == 3:
        return 'S'
    else:
...
def move_hard_coded():
    move_one_tile(tile_size=12)
    if detect_lack_of_progress():
        return
    move_one_tile(tile_size=12)
